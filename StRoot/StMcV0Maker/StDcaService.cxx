...
//the following two functions will calculate approximately the dca of two helices. 
//this is not a code for general purpose. use it when you understand the parameters 
//inside the code. for specific, the current parameters are tuned for STAR TPC track.
//Original version from Hui LONG, improved by Xianglei ZHU in May, 2008

//master switch for the minimization algorithm used in the case of connecting helices: 
//true -> StHelix method. slow, but accurate. just for checking purpose.
//false-> native minimization code. fast, and accurate enough.
bool kStHelixDca = false;

//switch for turn off the minimization in native code. this is OK for Lambda reconstruction.
//but minimization is essential for photon reconstruction. since adding minimization causes
//only minor lag in performance, we should keep it as default, even if for lambda reconstruction.
bool kMinimize   = false;	

//parameters to control the usage of minimization: for lambda recontruction 0.3, 0.3 is safe.
//for photon recontruction, 2, 2 is safe. By 'safe', I mean there is no signal loss compared to 
//the sthelix dca method. For photons, this is essential to keep all signals (with the 
//payment of performance), since we want to use same sign method to estimate the background. 
//there will be systematic deviation if the signal loss (due to limited accuracy) 
//is not the same for signals and background.
...
double closestDistance(const StPhysicalHelixD& helix1, const StPhysicalHelixD& helix2, double magnet, const StThreeVectorF& pv, StThreeVectorF& xv0, StThreeVectorF& op1, StThreeVectorF& op2 )
{
  ...
  if( d_p_pi>(r_p+r_pi)+2.0 || d_p_pi<fabs(r_p-r_pi)-2.0 )return 500.;//dca cut should be smaller 
  if( d_p_pi>=(r_p+r_pi) || d_p_pi<=fabs(r_p-r_pi) ){ //XZHU: it will be rare that two tracks with d_p_pi == r_p+r_pi come from a single particle decay. that means their momenta should be in the same direction or in opposite direction. it is normal for photon conversion to have two daughter electrons in the same direction. But it will be rare for two v0 decay daughters in the very opposite direction (that means the pt of v0 is 0). FIXME: please notice the hard cut 2.0cm above, you might change it for some purposes.
    fi_root1_p=0.;
    fi_root2_p=0.;
    if(d_p_pi>=(r_p+r_pi)){
	 fi_root1_pi=PI;
    ...
    if(kStHelixDca && (d_p_pi>=(r_p+r_pi)-kShiftConnect || d_p_pi<=fabs(r_p-r_pi)+kShiftContain)){	
	 pair<double,double> tmps = helix1.pathLengths(helix2); 
	 StThreeVectorF ox1 = helix1.at(tmps.first);
	 StThreeVectorF ox2 = helix2.at(tmps.second);
	 d = (ox1-ox2).mag();
  ...
  fi_root1_p=fi_root1_p-floor(fi_root1_p/2/PI)*2*PI; //XZHU: try floor() here, instead of (int). since a wide range of period is tried below, the result should be the same. in fact, fi_root1_p should always be less than 0 (0,-2pi for 1 period), since we are looking backward for the position where the helix originates. ADDED: it is not so simple. we should keep the code as it is.
  ...
  if((v0_root1[0]*v0_root1[0]+v0_root1[1]*v0_root1[1])<R2_TPC){ //XZHU: if the overlapping position is close (25 cm) to the outer layer of TPC. Even if the dca between two tracks is small, it is not interesting to us anymore. We find the right periods here, which is not necessary for particles with pt > 0.15GeV/c. in recHyperon, the period finding is not used. ADDED: lots of daughters has small pt. we do not understand how multiple periods tracks are recorded (split to multiple tracks or not?). if we scan periods, we could have multiple signals for a single lambda!!! hope this effect can be simulated in embedding analysis.
    rec_zij=1000.;
    n1=-((int)(fabs((proton->Zc-pv.z())/proton->Vz/2./PI))+1);
    n2=proton->Flag;  //XZHU: it is meanlingless to go forward in a helix to get the dca point where the particle (or helix) should orginates from! so i suggest to decrease this flag to 0, since fi_root1_pi is [-2pi,2pi]. ADDED: it is not so simple due to the TPC track momentum resolution. we should keep the code as it is.
...
void  dcaPToPi(float *fi_p,float *fi_pi, StTrackHelix* proton,StTrackHelix* pion,float *d_root,float *v0,float alfa)
{
  ...
  float old_d,oldx1,oldy1,oldz1,oldx2,oldy2,oldz2;

  Co_a=proton->r*proton->r+proton->Vz*proton->Vz;
  Co_b=-proton->h*pion->h*proton->r*pion->r*cos(alfa)-proton->Vz*pion->Vz;  //XZHU: bug fix to consider also the case of same charge helices
  Co_c=pion->r*pion->r+pion->Vz*pion->Vz;
  float Tp = proton->r*proton->h*sin(*fi_p*proton->h+proton->theta);
  float Kp = proton->r*proton->h*cos(*fi_p*proton->h+proton->theta);
  float Tn = pion->r*pion->h*sin(*fi_pi*pion->h+pion->theta);
  float Kn = pion->r*pion->h*cos(*fi_pi*pion->h+pion->theta);
  ...
  double d_p_pi=sqrt((pion->Xc-proton->Xc)*(pion->Xc-proton->Xc)+(pion->Yc-proton->Yc)*(pion->Yc-proton->Yc));
  double r_p=proton->r;   //radium of proton helix
  double r_pi=pion->r;    //radium of pion helix

  if(kMinimize && (d_p_pi>=(r_p+r_pi)-kShiftConnect || d_p_pi<=fabs(r_p-r_pi)+kShiftContain) ){  
     double dfi1, dfi2;
     ...
     double dfi1low = 0;
     double dfi1high = dfi_p/fabs(dfi_p)*TMath::Max((double)fabs(dfi_p),0.01);
     ...
     while(fabs(ddfi)>minddfi){
	  ...
	  for(dfi1=dfi1low;(dfi1-dfi1low)/ddfi<=10.05;dfi1+=ddfi){
	     ...
	     do{
		  ...
		  if(iter==0){
		     ...
		     M=sqrt((tx1-tx2)*(tx1-tx2)+(ty1-ty2)*(ty1-ty2)+(tz1-tz2)*(tz1-tz2));
		  }
		  ...
		  double dfn = -dtx2*dtx2+(tx1-tx2)*ddtx2-dty2*dty2+(ty1-ty2)*ddty2
		     -dtz2*dtz2+(tz1-tz2)*ddtz2;
	  ...
	  if(nshiftl>0&&nshiftr>0) {
	     ...
	     //cout<<"warn"<<nshiftl<<" "<<nshiftr<<" "<<ndig<<" "<<status<<" "<<10*ddfi<<" "<<dmin<<endl;
	     break;
	  }
     }
     //if(nshiftl>5||nshiftr>5)cout<<nshiftl<<" "<<nshiftr<<" "<<ndig<<" "<<status<<endl;
  ...
  old_d=sqrt((oldx1-oldx2)*(oldx1-oldx2)+(oldy1-oldy2)*(oldy1-oldy2)+(oldz1-oldz2)*(oldz1-oldz2));
...
double closestDistance(const StThreeVectorF& xv0, const StThreeVectorF& pv0, const StPhysicalHelixD& helix, double magnet, StThreeVectorF& xxi, StThreeVectorF& op )
{
    float pxK,pyK,pzK,x,y,z,r,pL,pxL,pyL,pzL,pLK,pK,sign,xc,yc;
    //float P1=0, P2=0, P3=0;
    float a1, b1, c1;
    float tanK = -1;
    float x0,y0,z0;

    a1=xv0.x(); // gx gy gz are the decay vertex position of V0
    b1=xv0.y();
    c1=xv0.z();
    pxL= pv0.x(); // get momentum of V0
    pyL= pv0.y();
    pzL= pv0.z();
    ...
    sign=-hrot; // get the helicity of the helix, assigned to be field_reverse*charge
    xc=(x+r*sign*pyK/pbach.perp());  // get the x of the center of the pion track //XZHU: why not gtrack->Xc ?
    yc=(y-r*sign*pxK/pbach.perp());  // get the Y of the center of the pion track
    pL=sqrt(pxL*pxL+pyL*pyL+pzL*pzL);
    pLK=pxL*pxK+pyL*pyK+pzL*pzK;
    float xroot=pyL*(pyL*(a1-xc)-(b1-yc)*pxL)/(pxL*pxL+pyL*pyL);
    float yroot=pxL*(pxL*(b1-yc)-(a1-xc)*pyL)/(pxL*pxL+pyL*pyL);
    float dmax=fabs(pxL*(b1-yc)-(a1-xc)*pyL)/sqrt(pxL*pxL+pyL*pyL);
         // xroot and yroot are the coordinates of the crossing points or the closest point between the pion circle and lambda line ( in 2D ), it is still on the lambda line
           // dmax is the distance from the center of the pion circle to the Lambda
    if(dmax>=(r+0.5)) return 999.0;
    float newx,newy,newz,sign0=-1,dsave=999.0;
    if(dmax>r){ // no 2d crossing points but not too far either, then the newx and newy are the coordinates of the 2D closest point to the Lambda line but still on the circle. //XZHU: should be rare, since it needs the momenta of lambda and K is in line with each other. 
        newx=r/dmax*xroot+xc;
        newy=r/dmax*yroot+yc;
        if(((newx-x)*pxK+(newy-y)*pyK)>0)sign0=1.0;
        newz=z+sign0*r*2*asin(sqrt((newx-x)*(newx-x)+(newy-y)*(newy-y))/2.0/r)*pzK/sqrt(pxK*pxK+pyK*pyK);
        float newzL=pzL/pyL*(newy-b1)+c1;   // from new x, new y , calculate the newz on the helix
        //dsave=sqrt((newzL-newz)*(newzL-newz)+dmax*dmax); //XZHU: BUG! change dmax -> dmax-r
        dsave=sqrt((newzL-newz)*(newzL-newz)+(dmax-r)*(dmax-r)); //XZHU: fixed! in this case, we can not use the straight line approximation below. because these two lines always cross each other at somewhere far far away. therefore, you will have unphysical '0' dca, some of them will add up to the background.
        //tanK=-1;
        tanK=0;
    ...
    else{//  two crssoing points, fa, fb, fc  just intermidiate numbers in calculation without any geometric meaning. newx, new y has the same meaning in case 1
        float fa=1+pxL*pxL/pyL/pyL;
        float fb=2*pxL*((a1-xc)-(b1-yc)*pxL/pyL)/pyL;
        float fc=((a1-xc)-(b1-yc)*pxL/pyL)*((a1-xc)-(b1-yc)*pxL/pyL)-r*r;
        float newy1=(-fb-sqrt(fb*fb-4*fa*fc))*0.5/fa;
        float newx1=pxL*(newy1-(b1-yc))/pyL+(a1-xc)+xc;
        newy1=newy1+yc;
        float newy2=(-fb+sqrt(fb*fb-4*fa*fc))*0.5/fa;
        float newx2=pxL*(newy2-(b1-yc))/pyL+(a1-xc)+xc;
        newy2=newy2+yc;
        float ds1=sqrt((newx1-x)*(newx1-x)+(newy1-y)*(newy1-y));
        float da1=ds1/r;
        sign0=-1,dsave=999;
        if((newx1-x)*pxK+(newy1-y)*pyK>0) sign0=1.0;
        float newz1=z+sign0*2.0*r*asin(da1/2.0)*pzK/sqrt(pxK*pxK+pyK*pyK);  //XZHU: only work in one period. for sign0=1, it is weird to check the orign point in the forward direction
        float  newzL1=pzL/pyL*(newy1-b1)+c1;
        float ds2=sqrt((newx2-x)*(newx2-x)+(newy2-y)*(newy2-y));
        float da2=ds2/r;
        sign0=-1;
        if((newx2-x)*pxK+(newy2-y)*pyK>0) sign0=1.0;
        float newz2=z+sign0*2.0*r*asin(da2/2.0)*pzK/sqrt(pxK*pxK+pyK*pyK);
        float  newzL2=pzL/pyL*(newy2-b1)+c1;
    ...
    float   newpx=-(yc-newy)/r*sign*sqrt(pxK*pxK+pyK*pyK);
    float    newpy=(xc-newx)/r*sign*sqrt(pxK*pxK+pyK*pyK);
    x=newx;
    y=newy;
    z=newz;
    pxK=newpx;
    pyK=newpy;// // update the momentum of the pion track at the new postion
        ...
        // JUne 19, 2003   here calculate the distance between two straigt line, ( helix pion is approximated as straight line at the new position calculated above. lambda is always a streight line.
    float aK=pxK/pK;
    float bK=pyK/pK;
    float cK=pzK/pK;
    float aL=pxL/pL;
    float bL=pyL/pL;
    float cL=pzL/pL;
    float eA=(aK*aK+bK*bK+cK*cK);
    float eB=-(aK*aL+bK*bL+cK*cL);
    float eC=cK*dsave;
    float eD=-eB;
    float eE=-(aL*aL+bL*bL+cL*cL);
    float eF=cL*dsave;
    float t1=(eC*eE-eF*eB)/(eA*eE-eB*eD);
    float t2=(eA*eF-eC*eD)/(eA*eE-eB*eD);
    float min_d=sqrt((aK*t1-aL*t2)*(aK*t1-aL*t2)+(bK*t1-bL*t2)*(bK*t1-bL*t2)+(cK*t1-cL*t2-dsave)*(cK*t1-cL*t2-dsave));
    //if(tanK==0)
	 //cout<<min_d<<" "<<fabs(dsave)<<" "<<(cK*t1-cL*t2-dsave)<<" "<<(bK*t1-bL*t2)<<" "<<(aK*t1-aL*t2)<<" "<<t1<<" "<<t2<<" "<<aK<<" "<<bK<<" "<<cK<<" "<<aL<<" "<<bL<<" "<<cL<<" "<<dmax<<" "<<r<<" "<<sqrt((newx+aK*t1-xc)*(newx+aK*t1-xc)+(newy+bK*t1-yc)*(newy+bK*t1-yc))<<endl;
    if(min_d<fabs(dsave)){
        x0=    (aK*t1+aL*t2)/2.0+newx;
        y0=    (bK*t1+bL*t2)/2.0+newy;
        z0=    (cK*t1+cL*t2+dsave)/2.0+newz;
        dsave=min_d;        // update the x0,y0,z0 as the dca point between Lambda and pion

        //XZHU: update the Kaon px, py at dca point, should not be necessary. just for consistent check!
        float newkx = aK*t1+newx;
        float newky = bK*t1+newy;
        float newr  = sqrt((newkx-xc)*(newkx-xc)+(newky-yc)*(newky-yc));
        float ptK = sqrt(pxK*pxK+pyK*pyK);
    ...
    //float PT = sqrt(pow( P1,2) + pow( P2,2));
